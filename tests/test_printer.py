...
from u1_filament_automation.firmware_compatibility import CANDIDATE_205, STOCK_205
...
class PrinterInstallerTests(unittest.TestCase):
    ...
    def test_205_sandbox_selects_candidate_and_restores_exact_205_stock(self):
        with tempfile.TemporaryDirectory() as temporary:
            target = LocalPrinterTarget(Path(temporary))
            target.path.parent.mkdir(parents=True)
            target.path.write_bytes(
                validated_asset("flow_calibrator_stock_205.py", STOCK_205)
            )
            target.seed_config_if_missing()
            ...
            self.assertEqual(result.calibrator.sha256, CANDIDATE_205)
            self.assertNotEqual(result.calibrator.sha256, V6_SHA256)
            self.assertEqual(result.macro.state, "fw205-installed")
            self.assertEqual(
                sha256_bytes(target.read_path_bytes(ADAPTIVE_PA_MACRO_PATH)),
                ADAPTIVE_PA_MACRO_205_SHA256,
            )
            flow_write = next(
                item for item in result.writes if item.path == str(target.path)
            )
            self.assertEqual(flow_write.sha256_before, STOCK_205)
            self.assertEqual(flow_write.sha256_after, CANDIDATE_205)
            self.assertEqual(
                sha256_bytes(Path(flow_write.backup_path).read_bytes()), STOCK_205
            )

            restored = target.restore_atomic(
                flow_write.backup_path, CANDIDATE_205
            )
            self.assertEqual(restored.sha256_after, STOCK_205)
            self.assertEqual(target.path.read_bytes(), bundled_asset(
