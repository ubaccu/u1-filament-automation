...
from .firmware_compatibility import (
    CANDIDATE_205,
    MACRO_205_SHA256,
    MACRO_205_TESTED_SHA256,
    STOCK_205,
    TESTED_205,
)
...
ADAPTIVE_PA_MACRO_205_SHA256 = MACRO_205_SHA256
ADAPTIVE_PA_MACRO_205_TESTED_SHA256 = MACRO_205_TESTED_SHA256
...
class AdaptivePAMacroStatus:
    ...
    def complete(self) -> bool:
        return (
            self.state in {"v6-installed", "fw205-installed"}
            and self.loaded_by_klipper is True
        )
...
class PrinterSafetyStatus:
    ...
    def safe_to_modify(self) -> bool:
        return (
            ...
            and (
                self.machine_state is not None
                and self.machine_state.strip().casefold() in SAFE_MACHINE_STATES
            )
        )
...
def classify_calibrator(data: bytes, path: str) -> CalibratorStatus:
    ...
    if digest in {STOCK_SHA256, STOCK_205}:
        state = "stock-compatible"
    elif digest == V6_SHA256:
        state = "v6-installed"
    elif digest in {CANDIDATE_205, TESTED_205}:
        state = "fw205-installed"
    else:
        state = "unknown-blocked"
    return CalibratorStatus(path=path, sha256=digest, state=state)


def calibrator_install_spec(calibrator_sha256: str) -> tuple[str, str, str]:
    """Return asset, validated stock hash and installed hash for one firmware line."""
    if calibrator_sha256 in {STOCK_SHA256, V6_SHA256}:
        return "flow_calibrator_v6.py", STOCK_SHA256, V6_SHA256
    if calibrator_sha256 in {STOCK_205, CANDIDATE_205}:
        return "flow_calibrator_205_candidate.py", STOCK_205, CANDIDATE_205
    if calibrator_sha256 == TESTED_205:
        return "flow_calibrator_205_candidate.py", STOCK_205, TESTED_205
    raise PrinterInstallError(
        "Versione del calibratore non riconosciuta: configurazione bloccata"
    )


def macro_install_spec(calibrator_sha256: str) -> tuple[str, str, str]:
    """Return macro asset, packaged hash and expected status for one firmware line."""
    if calibrator_sha256 in {STOCK_SHA256, V6_SHA256}:
        return "adaptive_pa_macro.cfg", ADAPTIVE_PA_MACRO_SHA256, "v6-installed"
    if calibrator_sha256 in {STOCK_205, CANDIDATE_205, TESTED_205}:
        return (
            "adaptive_pa_macro_205.cfg",
            ADAPTIVE_PA_MACRO_205_SHA256,
            "fw205-installed",
        )
    raise PrinterInstallError(
        "Versione del calibratore non riconosciuta: configurazione bloccata"
...
def inspect_adaptive_pa_macro(
    ...
    digest = sha256_bytes(data)
    if digest == ADAPTIVE_PA_MACRO_SHA256:
        state = "v6-installed"
    elif digest in {
        ADAPTIVE_PA_MACRO_205_SHA256,
        ADAPTIVE_PA_MACRO_205_TESTED_SHA256,
    }:
        state = "fw205-installed"
    else:
        state = "unknown-blocked"
...
def plan_printer_setup(
    ...
    if isinstance(target, SSHPrinterTarget):
        from .firmware_compatibility import require_live_firmware
        require_live_firmware(target)
    ...
    _, _, expected_macro_state = macro_install_spec(calibrator.sha256)
    if macro.state not in {"missing", expected_macro_state}:
        raise PrinterInstallError(
            "Macro Adaptive PA riconosciuta ma appartenente a un'altra linea firmware"
        )
...
def install_printer_setup(
    ...
    asset_name, stock_sha256, installed_sha256 = calibrator_install_spec(
        plan.calibrator.sha256
    )
    macro_asset, macro_sha256, expected_macro_state = macro_install_spec(
        plan.calibrator.sha256
    )
    macro_data = validated_asset(macro_asset, macro_sha256)
    calibrator_data = validated_asset(asset_name, installed_sha256)
    validate_python_source(calibrator_data, str(FLOW_CALIBRATOR_PATH))
    ...
    try:
        ...
        if plan.install_calibrator:
            flow_write = target.install_atomic(stock_sha256, calibrator_data)
            writes.append(flow_write)
    except Exception as exc:
        rollback_errors: list[str] = []
        if flow_write is not None:
            try:
                target.restore_atomic(flow_write.backup_path, installed_sha256)
        ...
        if macro_write is not None:
            try:
                target.delete_path_if_sha(
                    ADAPTIVE_PA_MACRO_PATH, macro_sha256
                )
            except Exception as rollback_exc:
    ...
    if installed_calibrator.sha256 != installed_sha256:
        raise PrinterInstallError("Verifica finale calibratore AutoPA fallita")
    if installed_macro.state != expected_macro_state:
...
class LocalPrinterTarget:
    ...
    def restore_atomic(
        ...
        _, expected_backup_sha256, _ = calibrator_install_spec(
...
"""
    ...
    def install_atomic(self, expected_sha256: str, new_data: bytes) -> FileWriteResult:
        ...
        script = _remote_install_script(
            str(self.remote_path), expected_sha256, sha256_bytes(new_data), encoded
        )
...
try:
    ...
    if hashlib.sha256(new_data).hexdigest() != expected_new:
        raise RuntimeError("asset AutoPA non valido")
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", SyntaxWarning)
...
class MoonrakerClient:
    ...
    def safety_status(self) -> PrinterSafetyStatus:
        ...
        if isinstance(machine, dict) and machine.get("main_state") is not None:
            value = machine["main_state"]
            if type(value) in (int, str):
                machine_state = str(value)
        active = virtual_sdcard.get("is_active")
        if type(active) is not bool:
            raise PrinterInstallError("Stato virtual_sdcard non valido / Invalid virtual_sdcard state")
