# Snapmaker U1 flow calibration framework
# Adaptive PA / U1FA integration update
...
class FlowCalibrator(object):
    ...
    def _measure_k(self, extruder, inductance_coil, k, params, extruder_dir=None):
        ...
        try:
            ...
            # APA/U1FA measurement-only stabilization. Snapmaker's stock per-K
            # cleanup briefly sets the nozzle target to 170C; after the blocking
            # return to the requested calibration temperature, allow the melt zone
            # to settle before sampling the next K. Native FLOW_CALIBRATE remains
            # unchanged because its stabilization time is zero.
            apa_stabilize_time = float(params.get('apa_stabilize_time', 0.0) or 0.0)
            if apa_stabilize_time > 0.0:
                self._toolhead.dwell(apa_stabilize_time)
    ...
    CHAIN       -> 1 enables chained APA cells without closing calibration between cells
    CHAIN_LAST  -> 0 intermediate cell, 1 final cell (normal stock final cleanup)
    APA_MEASURE_ONLY -> 1 returns measured K without applying/saving it (APA/U1FA)
    """
    def cmd_FLOW_CALIBRATE(self, gcmd):
        ...
        apa_measure_only = gcmd.get_int('APA_MEASURE_ONLY', 0, minval=0, maxval=1)
        apa_stabilize_time = gcmd.get_float(
            'APA_STABILIZE', 3.0 if apa_measure_only else 0.0, minval=0.0, maxval=30.0)
        ...
        cali_params = {
            ...
            'apa_stabilize_time': apa_stabilize_time
        }
        ...
        if apa_measure_only and apa_stabilize_time > 0.0:
            gcmd.respond_info(
                f'[flow_calibrate] APA MEASURE ONLY: thermal stabilization {apa_stabilize_time:.1f}s before each K sample')
        if chain_mode:
            gcmd.respond_info(
                f'[flow_calibrate] APA CHAIN: CHAIN=1 CHAIN_LAST={chain_last} '
                f'ALGORITHM={algorithm} CALC_MODE={cali_params["calc_mode"]} '
                f'OS_COUNT={cali_params["os_sample_count"]} '
                f'MEASURE_ONLY={apa_measure_only} '
                f'ACTIVE_BEFORE={1 if self._apa_chain_active else 0}')
        ...
        try:
            ...
            backup_pa = estepper.pressure_advance
            ...
            try:
                ...
                # Continue only after a successful intermediate CHAIN cell.
                # This is algorithm-agnostic: stock 2.0.0 computes measure_success_k
                # for LINEAR_FITTING as well as OS_ACCEL/OS_DECEL/OS_ACCEL_DECEL.
                chain_continue = bool(
                    chain_mode == 1 and chain_last == 0
                    and not self._abort_calibration
                    and measure_success_k is not None)

                # Normal commands, final CHAIN cell, and failures retain stock end.
                # Valid intermediate cells keep the begin/end lifecycle open.
                ...
                if not self._abort_calibration and measure_success_k != None:
                    ...
                    if apa_measure_only:
                        ...
                        gcmd.respond_info(
                            '[flow_calibrate] APA MEASURE ONLY: measured K returned; '
                            'native/internal flow K not modified')
                    else:
                    ...
                    if chain_continue:
                        # _measure_k() still performs Snapmaker's normal per-K
                        # discard/rough-clean sequence. Only final calibration
                        # cleanup is deferred until CHAIN_LAST=1.
                        gcmd.respond_info(
                            "[flow_calibrate] APA CHAIN: intermediate cell complete; "
                            "stock 2.0.0 per-K cleaning preserved, keeping hotend/calibration active")
                    else:
                        self._end_of_calibration(extruder)
                ...
                # measure failure
                    ...
                    if self._abort_reason == ABORT_REASON_OUT_OF_RANGE:
                        ...
                        if not apa_measure_only:
                            ...
                            gcmd.respond_info(
                                '[flow_calibrate] APA MEASURE ONLY: out-of-range result not persisted')
                        self._end_of_calibration(extruder)

                # Every APA measurement temporarily changes pressure advance while
                # probing candidate K values. Restore exactly the PA that was active
                # before this cell so U1FA/Orca runtime PA remains authoritative.
                if apa_measure_only and backup_pa is not None:
                    self._set_pressure_advance(extruder, backup_pa, backup_st)
                    gcmd.respond_info(
                        f'[flow_calibrate] APA MEASURE ONLY: restored previous PA {backup_pa:.6f}')

                self._abort_calibration = False
                self._abort_reason = None

                if chain_continue:
                    # Restore the requested target non-blocking so the next cell
                    # can start immediately while remaining over the discard area.
