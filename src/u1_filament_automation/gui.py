...
class CalibrationController:
    def __init__(
        ...
        # A live calibration requires a successful read-only firmware check in
        # the current session.  This prevents a retained legacy v6 file from
        # being used after the printer firmware has changed.
    ...
    def start(self, selection: CalibrationSelection) -> None:
        if self.ssh_target and not self._firmware_checked:
            ...
            self._set_job(JobSnapshot("blocked", message, selection))
...
<div class="card"><p class="warn"><strong>{_tr(language, 'Aggiornamenti firmware:', 'Firmware updates:')}</strong> {_tr(language, 'un aggiornamento può rimuovere U1FA AutoPA Mod. Esegui prima questo controllo in sola lettura. Il firmware U1 2.0.0.205 è supportato con controlli protetti: installazione e calibrazione vengono abilitate soltanto quando versione e SHA-256 dei componenti corrispondono esattamente a quelli convalidati.', 'an update may remove U1FA AutoPA Mod. Run this read-only check first. U1 firmware 2.0.0.205 is supported with guarded checks: installation and calibration are enabled only when the firmware version and component SHA-256 values exactly match the validated set.')}</p></div>
...
<script>
var picker=document.getElementById('color-picker'),hexField=document.getElementById('color-hex'),sample=document.getElementById('color-sample'),mode=document.getElementById('color-mode'),multiWrap=document.getElementById('multi-color-wrap'),multiField=document.getElementById('multi-color-hexes'),multiList=document.getElementById('multi-color-list'),addMulti=document.getElementById('add-multi-color'),multiColors=['#D9A62E','#D8494A'];
...
function validColor(value){{var normalized=value.trim().toUpperCase();if(normalized.charAt(0)!=='#')normalized='#'+normalized;return /^#[0-9A-F]{{6}}$/.test(normalized)?normalized:'#FFFFFF';}}
function syncMulti(){{multiField.value=multiColors.map(validColor).join(',');}}
