...
<div class="card"><p class="warn"><strong>{_tr(language, 'Aggiornamenti firmware:', 'Firmware updates:')}</strong> {_tr(language, 'un aggiornamento può rimuovere U1FA AutoPA Mod. Esegui prima questo controllo in sola lettura. Il firmware U1 2.0.0.205 è riconosciuto per diagnosi e test sandbox; installazione e calibrazione reali restano bloccate in questa beta.', 'an update may remove U1FA AutoPA Mod. Run this read-only check first. U1 firmware 2.0.0.205 is recognized for diagnostics and sandbox tests; live installation and calibration remain blocked in this beta.')}</p></div>
