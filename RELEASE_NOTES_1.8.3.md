...
- validazione end-to-end eseguita su **U1 reale + Snapmaker Orca 2.4.0**: durante la stampa Fluidd/Klipper ha riportato valori `pressure_advance` dinamici e differenti nel tempo, confermando che il modello Adaptive PA scritto da U1FA viene letto dallo slicer e applicato realmente dalla stampante;
...
- end-to-end validation completed on a **real U1 with Snapmaker Orca 2.4.0**: during printing, Fluidd/Klipper reported different `pressure_advance` values over time, confirming that the Adaptive PA model written by U1FA is read by the slicer and actually applied by the printer;
